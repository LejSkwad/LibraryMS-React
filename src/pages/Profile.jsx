  ...
  async function changePassword(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (serverMsg.toLowerCase().includes('wrong password')) {
          setErrors({ oldPassword: 'Mật khẩu hiện tại không đúng.' });
        } else if (serverMsg.toLowerCase().includes('do not match')) {
        ...
        <div className="card profile-card">
          ...
          <div className="card-body profile-card-body">
            <div className="profile-avatar-section">
              ...
              <div>
                <div className="profile-name-heading">{fullName || 'N/A'}</div>
                {p && <span className={`badge ${roleBadge}`}>{p.role}</span>}
              </div>
            </div>
            <div className="profile-info-list">
              <div className="profile-info-row">
                <span className="profile-label">Số CCCD</span>
                <span>{p?.socialNumber || 'N/A'}</span>
              </div>
              <div className="profile-info-row">
                <span className="profile-label">Họ</span>
                <span>{p?.firstName || 'N/A'}</span>
              </div>
              <div className="profile-info-row">
                <span className="profile-label">Tên</span>
                <span>{p?.lastName || 'N/A'}</span>
              </div>
              <div className="profile-info-row">
        ...
        <div className="card profile-card">
          ...
          <div className="card-body profile-card-body">
            <form onSubmit={changePassword} className="form-stack">
              ...
              {msg.text && (
                <div className={`change-pw-msg ${msg.type === 'success' ? 'text-success' : 'text-danger'}`}>
                  {msg.text}
                </div>
              )}
